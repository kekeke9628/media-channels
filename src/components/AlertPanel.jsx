...
import { ALERT_DAYS, removeIn, lateLabel } from '../constants.js';
...
export default function AlertPanel({ alerts, kpi, isEditor, refDate, onRemove, onPick }) {
        ...
        {alerts.stale.length > 0 && (
          <>
            ...
            <div className="slack danger">
              ...
              <span>{alerts.stale.slice(0, 4).map((o) => `[${o.name}] ${o.overdue.brand} ${lateLabel(o.overdue, refDate)}`).join(' / ')}{alerts.stale.length > 4 ? ` 외 ${alerts.stale.length - 4}건` : ''}</span>
            </div>
            {/* 조치는 건별로 해야 하므로 아래에 따로 편다. */}
            {alerts.stale.map((o) => row(o, lateLabel(o.overdue, refDate), o.overdue))}
          </>
        )}
      </section>
